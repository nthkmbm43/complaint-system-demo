...
export default function StaffManagementPage() {
  ...
  return (
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      {showModal && (
        <div 
          className="fixed inset-0 z-[1000] overflow-y-auto bg-slate-900/70 backdrop-blur-md flex justify-center p-4 py-12 md:py-20"
          onClick={closeModal}
        >
          <div 
            className="relative bg-white rounded-[3rem] w-full max-w-3xl shadow-[0_20px_70px_-10px_rgba(0,0,0,0.5)] p-10 md:p-16 my-auto animate-in zoom-in-95 duration-300"
            onClick={(e) => e.stopPropagation()}
          >
            {/* Elegant Header - Balanced Spacing */}
            <div className="flex items-center justify-between mb-16">
              <div className="flex items-center gap-6">
                <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center text-4xl shadow-sm border border-indigo-100">👤</div>
                <div>
                  <h3 className="text-4xl font-black text-slate-900 tracking-tight">{editingStaff ? "แก้ไขข้อมูลเจ้าหน้าที่" : "เพิ่มเจ้าหน้าที่ใหม่"}</h3>
                  <p className="text-sm font-bold text-slate-400 uppercase tracking-[0.3em] mt-2 ml-1">Staff Administration</p>
                </div>
              </div>
              <button onClick={closeModal} className="w-14 h-14 rounded-2xl bg-slate-50 text-slate-400 hover:text-red-500 hover:bg-red-50 transition-all flex items-center justify-center border border-slate-100">
            ...
            <form onSubmit={handleSubmit} className="space-y-12">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                <div className="space-y-3">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">Username / ID</label>
                  <input type="text" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} disabled={!!editingStaff} required placeholder="เช่น teacher_01" className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 focus:outline-none focus:ring-8 focus:ring-indigo-500/5 transition-all text-lg" />
                </div>
                <div className="space-y-3">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">รหัสผ่าน {editingStaff && "(ปล่อยว่างถ้าไม่เปลี่ยน)"}</label>
                  <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required={!editingStaff} placeholder="••••••••" className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold focus:outline-none focus:ring-8 focus:ring-indigo-500/5 transition-all text-lg" />
                </div>
                <div className="space-y-3 sm:col-span-2">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">ชื่อ-นามสกุล</label>
                  <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="กรอกชื่อและนามสกุล" className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 focus:outline-none focus:ring-8 focus:ring-indigo-500/5 transition-all text-lg" />
                </div>
                <div className="space-y-3">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">ระดับสิทธิ์</label>
                  <div className="relative">
                    <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 appearance-none focus:ring-8 focus:ring-indigo-500/5 transition-all text-lg">
                ...
                <div className="space-y-3">
                  <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">คณะ</label>
                  <div className="relative">
                    <select value={form.faculty} onChange={(e) => setForm({ ...form, faculty: e.target.value, major: "" })} className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 appearance-none focus:ring-8 focus:ring-indigo-500/5 transition-all text-lg">
                ...
                {form.faculty && (
                  <div className="space-y-3 sm:col-span-2">
                    <label className="text-xs font-black text-slate-500 uppercase tracking-widest ml-1">สาขาวิชา</label>
                    <div className="relative">
                      <select value={form.major} onChange={(e) => setForm({ ...form, major: e.target.value })} className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-700 appearance-none focus:ring-8 focus:ring-indigo-500/5 transition-all text-lg">
              ...
              <div className="flex flex-col sm:flex-row gap-6 pt-10">
                <button type="button" onClick={closeModal} className="flex-1 py-6 bg-white border-2 border-slate-100 text-slate-500 font-black rounded-[2rem] hover:bg-slate-50 transition-all uppercase text-sm tracking-widest">ยกเลิก</button>
                <button type="submit" disabled={saving} className="flex-[2] py-6 bg-slate-900 hover:bg-indigo-600 text-white font-black rounded-[2rem] transition-all uppercase text-sm tracking-widest shadow-2xl shadow-indigo-500/30 disabled:opacity-50 hover:scale-[1.02] active:scale-95">{saving ? "กำลังบันทึก..." : editingStaff ? "ยืนยันการแก้ไขข้อมูล" : "🛡️ ยืนยันเพิ่มเจ้าหน้าที่"}</button>
              </div>
            </form>
          </div>
        </div>
