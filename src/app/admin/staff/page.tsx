...
export default function StaffManagementPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const result = await Swal.fire({
      title: `ยืนยันการ${editingStaff ? "แก้ไข" : "เพิ่ม"}ข้อมูลเจ้าหน้าที่`,
    ...
    <div className="space-y-6 pb-20 animate-in fade-in duration-500">
      ...
      {showModal && (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center">
          {/* Layer 1: Permanent Full-Screen Backdrop (No gaps) */}
          <div 
          ...
          >
            <div 
              className="relative bg-white rounded-[2.5rem] w-full max-w-2xl shadow-[0_30px_100px_-20px_rgba(0,0,0,0.5)] p-8 md:p-10 my-auto animate-in zoom-in-95 duration-300"
              ...
              <div className="flex items-start justify-between mb-12">
                <div className="flex items-center gap-6">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] font-black text-slate-400 uppercase tracking-[0.2em] mt-1 ml-0.5">Staff Access Setup</p>
                  </div>
                </div>
                <button onClick={closeModal} className="flex items-center gap-2 px-5 py-2.5 bg-slate-50 hover:bg-slate-100 text-slate-400 rounded-xl transition-all border border-slate-100">
              ...
              <form onSubmit={handleSubmit} className="space-y-10">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-1">Username / ID</label>
                    <input type="text" value={form.username} onChange={(e) => setForm({ ...form, username: e.target.value })} disabled={!!editingStaff} required placeholder="เช่น teacher_01" className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-600 focus:outline-none focus:ring-8 focus:ring-slate-100 transition-all text-lg" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-1">รหัสผ่าน {editingStaff && "(ปล่อยว่างถ้าไม่เปลี่ยน)"}</label>
                    <input type="password" value={form.password} onChange={(e) => setForm({ ...form, password: e.target.value })} required={!editingStaff} placeholder="••••••••" className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold focus:outline-none focus:ring-8 focus:ring-slate-100 transition-all text-lg" />
                  </div>
                  <div className="space-y-3 sm:col-span-2">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-1">ชื่อ-นามสกุล</label>
                    <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} required placeholder="กรอกชื่อและนามสกุล" className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-600 focus:outline-none focus:ring-8 focus:ring-slate-100 transition-all text-lg" />
                  </div>
                  <div className="space-y-3">
                    <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-1">ระดับสิทธิ์</label>
                    <div className="relative">
                      <select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value })} className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-600 appearance-none focus:ring-8 focus:ring-slate-100 transition-all text-lg">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="relative">
                      <select value={form.faculty} onChange={(e) => setForm({ ...form, faculty: e.target.value, major: "" })} className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-600 appearance-none focus:ring-8 focus:ring-slate-100 transition-all text-lg">
                  ...
                  {form.faculty && (
                    <div className="space-y-3 sm:col-span-2">
                      <label className="text-[10px] font-black text-slate-900 uppercase tracking-widest ml-1">สาขาวิชา</label>
                      <div className="relative">
                        <select value={form.major} onChange={(e) => setForm({ ...form, major: e.target.value })} className="w-full px-6 py-5 bg-slate-50 border border-slate-100 rounded-2xl font-bold text-slate-600 appearance-none focus:ring-8 focus:ring-slate-100 transition-all text-lg">
                ...
                <div className="flex flex-col sm:flex-row gap-6 pt-10 border-t border-slate-50">
                  ...
                  >
                     ...
                     <span>{saving ? "กำลังดำเนินการ..." : editingStaff ? "ยืนยันการบันทึกข้อมูล" : "ยืนยันเพิ่มเจ้าหน้าที่"}</span>
                  </button>
                </div>
              </form>
            </div>
