...
export default function TutorialVideoPlayer() {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      setProgress((p) => {
        ...
        return p + 2; // 50 steps × 100ms = 5 seconds per slide
      });
    }, 100);
  ...
  return (
    <div className="lg:col-span-2 bg-slate-900 rounded-[3rem] overflow-hidden relative group aspect-video flex flex-col">
      {/* Screenshot Display */}
      <div className="relative flex-1 overflow-hidden">
        ...
        <div className="absolute bottom-0 left-0 right-0 h-1 bg-white/10">
          <div
            ...
            style={{ width: `${isPlaying ? progress : ((current / STEPS.length) * 100)}%` }}
          />
        </div>
      </div>
      ...
      <div className="bg-slate-800/80 backdrop-blur px-6 py-4 flex items-center gap-4 border-t border-white/5">
        ...
        {/* Full Screen hint */}
        <a
          href={`/api/tutorial-step/${step.key}`}
          target="_blank"
          className="w-8 h-8 bg-white/10 hover:bg-white/20 rounded-full flex items-center justify-center transition-colors shrink-0"
          title="ดูแบบเต็มจอ"
        >
          <svg className="w-3.5 h-3.5 fill-white" viewBox="0 0 24 24"><path d="M7 14H5v5h5v-2H7v-3zm-2-4h2V7h3V5H5v5zm12 7h-3v2h5v-5h-2v3zM14 5v2h3v3h2V5h-5z"/></svg>
        </a>
      </div>
    </div>
  );
